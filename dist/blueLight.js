...
async function main() {
    ...
    servient.addClientFactory(new binding_http_1.HttpClientFactory({ allowSelfSigned: true }));
    servient.addClientFactory(new binding_http_1.HttpsClientFactory({ allowSelfSigned: true }));
    ...
    try {
        ...
        const blueLightTD = allTDs.find((td) => td.title === "virtualLightBlue");
        ...
        debugLog("VirtualColorSensor objectPresence href:", hrefFor(colorSensorTD, "properties", "objectPresence"));
        debugLog("VirtualColorSensor color href:", hrefFor(colorSensorTD, "properties", "color"));
        debugLog("virtualLightBlue lightState href:", hrefFor(blueLightTD, "properties", "lightState"));
        // Consume Things
        ...
        try {
            ...
            if (fs.existsSync(hueTdPath)) {
                ...
                hueLight = await WoTHelpers.consume(hueTdJson);
            ...
            console.log("ℹ Philips Hue light not available - virtual light only mode:", err instanceof Error ? err.message : String(err));
        }
        ...
        const checkInterval = setInterval(async () => {
            try {
                ...
                const isPresent = (await presence.value());
                const now = Date.now();
                if (DEBUG && now - lastHeartbeat >= 2000) {
                    debugLog("objectPresence:", isPresent, "blueCubeDetected:", blueCubeDetected);
                    lastHeartbeat = now;
                }
