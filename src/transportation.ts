...
const UARM_POSITION_TOLERANCE = Number.parseFloat(
	process.env.UARM_POSITION_TOLERANCE || "0.020",
);
/** Maximum timeout for Uarm motion commands before raising an error (default: 30 seconds) */
const UARM_MOVE_TIMEOUT_MS = Number.parseInt(
	process.env.UARM_MOVE_TIMEOUT_MS || "30000",
	10,
);
/** Position polling interval for closed-loop control feedback (default: 100ms) */
const UARM_POLL_INTERVAL_MS = Number.parseInt(
	process.env.UARM_POLL_INTERVAL_MS || "100",
	10,
);
...
function normalizeUnit(unit: unknown): string {
	...
	if (
		...
		return "millimeter";
	if (u === "degree" || u === "degrees" || u === "deg") return "degree";
	if (u === "radian" || u === "radians" || u === "rad") return "radian";
...
function convertFromMeters(valueMeters: number, unit: string): number {
	const normalizedUnit = normalizeUnit(unit);
	if (normalizedUnit === "centimeter") return valueMeters * 100;
	if (normalizedUnit === "millimeter") return valueMeters * 1000;
	return valueMeters; // meter
}

function convertFromDegrees(valueDegrees: number, unit: string): number {
	const normalizedUnit = normalizeUnit(unit);
	if (normalizedUnit === "radian") return valueDegrees * (Math.PI / 180);
 ...
 */
function convertPosition(pos: Position, td: WoT.ThingDescription): Position {
...
function isUarm(robotTD: WoT.ThingDescription): boolean {
	return robotTD.title?.includes("Uarm") || false;
}

function convertUarmGoToInput(
	posMeters: { x: number; y: number; z: number },
	uarmTD: WoT.ThingDescription,
): { x: number; y: number; z: number } {
	const goTo = (uarmTD.actions as any)?.goTo;
	...
	const convertLinear = (value: number, unit: string): number => {
 ...
 */
async function waitForUarmAt(
	uarm: WoT.ConsumedThing,
	uarmTD: WoT.ThingDescription,
	targetMeters: { x: number; y: number; z: number },
	options?: { tolerance?: number; timeoutMs?: number; pollMs?: number },
): Promise<void> {
	...
	let lastObserved: { x: number; y: number; z: number } | undefined;
	let lastObservedRaw: { x: number; y: number; z: number } | undefined;
	let lastLog = 0;

	// Extract position feedback unit specifications from Thing Description
	const curPosProps = (uarmTD.properties as any)?.currentPosition?.properties;
	const curXUnit = curPosProps?.x?.unit || "meter";
	const curYUnit = curPosProps?.y?.unit || "meter";
	const curZUnit = curPosProps?.z?.unit || "meter";

	const toMeters = (v: number, unit: string): number =>
		convertToMeters(v, unit);
	while (Date.now() - start < timeoutMs) {
		let cur: any;
		try {
			const curProp = await uarm.readProperty("currentPosition");
			cur = (await curProp.value()) as any;
		} catch (err) {
			/**
...
async function uarmGoToAndWait(
	uarm: WoT.ConsumedThing,
	uarmTD: WoT.ThingDescription,
	posMeters: { x: number; y: number; z: number },
	options?: { tolerance?: number; timeoutMs?: number; pollMs?: number },
	...
	// Debug logging
	console.log(
		`[DEBUG_UARM] GoTo Target(M): ${JSON.stringify(
			posMeters,
 ...
 */
async function detectColor(colorSensor: WoT.ConsumedThing): Promise<string> {
...
async function main() {
	...
	try {
		...
		const uarm1TD = allTDs.find((td: any) => td.title === "VirtualUarm1");
		const uarm2TD = allTDs.find((td: any) => td.title === "VirtualUarm2");
		const ur3TD = allTDs.find((td: any) => td.title === "VirtualUR3");

		const conveyor1TD = allTDs.find(
			(td: any) => td.title === "VirtualConveyorBelt1",
		);
		const conveyor2TD = allTDs.find(
			(td: any) => td.title === "VirtualConveyorBelt2",
		);
		...
		const sensor1TD = allTDs.find(
			(td: any) => td.title === "VirtualInfraredSensor1",
		);
		const sensor2TD = allTDs.find(
			(td: any) => td.title === "VirtualInfraredSensor2",
		);
		const sensor3TD = allTDs.find(
			(td: any) => td.title === "VirtualInfraredSensor3",
		);
		const sensor4TD = allTDs.find(
			(td: any) => td.title === "VirtualInfraredSensor4",
		);
		...
		// Consume all Things
		const uarm1 = await WoTHelpers.consume(uarm1TD as WoT.ThingDescription);
		const uarm2 = await WoTHelpers.consume(uarm2TD as WoT.ThingDescription);
		const ur3 = await WoTHelpers.consume(ur3TD as WoT.ThingDescription);

		const conveyor1 = await WoTHelpers.consume(
			conveyor1TD as WoT.ThingDescription,
		);
		const conveyor2 = await WoTHelpers.consume(
			conveyor2TD as WoT.ThingDescription,
		);

		const colorSensor = await WoTHelpers.consume(
			colorSensorTD as WoT.ThingDescription,
		);

		const sensor1 = sensor1TD
			? await WoTHelpers.consume(sensor1TD as WoT.ThingDescription)
			: null;
		const sensor2 = sensor2TD
			? await WoTHelpers.consume(sensor2TD as WoT.ThingDescription)
			: null;
		const sensor3 = sensor3TD
			? await WoTHelpers.consume(sensor3TD as WoT.ThingDescription)
			: null;
		const sensor4 = sensor4TD
			? await WoTHelpers.consume(sensor4TD as WoT.ThingDescription)
			: null;
		...
		// Uarm1 units
		const u1GoTo = (uarm1TD.actions as any)?.goTo;
		const u1CurPos = (uarm1TD.properties as any)?.currentPosition;
		console.log("VirtualUarm1:");
		console.log(
			`  goTo input: x=${u1GoTo?.input?.properties?.x?.unit}, y=${u1GoTo?.input?.properties?.y?.unit}, z=${u1GoTo?.input?.properties?.z?.unit}`,
		);
		console.log(
			`  currentPosition: x=${u1CurPos?.properties?.x?.unit}, y=${u1CurPos?.properties?.y?.unit}, z=${u1CurPos?.properties?.z?.unit}`,
		);

		// Uarm2 units
		const u2GoTo = (uarm2TD.actions as any)?.goTo;
		const u2CurPos = (uarm2TD.properties as any)?.currentPosition;
		console.log("VirtualUarm2:");
		console.log(
			`  goTo input: x=${u2GoTo?.input?.properties?.x?.unit}, y=${u2GoTo?.input?.properties?.y?.unit}, z=${u2GoTo?.input?.properties?.z?.unit}`,
		);
		console.log(
			`  currentPosition: x=${u2CurPos?.properties?.x?.unit}, y=${u2CurPos?.properties?.y?.unit}, z=${u2CurPos?.properties?.z?.unit}`,
		);

		// UR3 units
		const ur3GoToPos = (ur3TD.actions as any)?.goToPosition;
		console.log("VirtualUR3:");
		console.log(
			`  goToPosition input: x=${ur3GoToPos?.input?.properties?.x?.unit}, y=${ur3GoToPos?.input?.properties?.y?.unit}, z=${ur3GoToPos?.input?.properties?.z?.unit}`,
		);
		console.log(
			`  goToPosition angles: rx=${ur3GoToPos?.input?.properties?.rx?.unit}, ry=${ur3GoToPos?.input?.properties?.ry?.unit}, rz=${ur3GoToPos?.input?.properties?.rz?.unit}`,
		);

		// Sensor units
		if (sensor1TD) {
			const s1ObjDist = (sensor1TD.properties as any)?.objectDistance;
			console.log(
				`VirtualInfraredSensor1: objectDistance unit=${s1ObjDist?.unit}`,
			);
		}
		if (sensor3TD) {
			const s3ObjDist = (sensor3TD.properties as any)?.objectDistance;
			console.log(
				`VirtualInfraredSensor3: objectDistance unit=${s3ObjDist?.unit}`,
			);
		}
		console.log(
			"===============================================================\n",
		...
		const spawnPickPosition: Position = {
		...
		const spawnLiftPosition: Position = {
		...
		const conveyor1AbovePosition: Position = {
		...
		const conveyor1PlacePosition: Position = {
		...
		const uarm2HomePosition: Position = {
		...
		const conveyor1PickPosition: Position = {
			x: 1.205,
			y: 1.5675,
			z: 1.08,
			rx: 0,
		...
		const conveyor2DropPosition: Position = {
		...
		const conveyor2PickPosition: Position = {
		...
		const ur3IntermediatePosition: Position = {
		...
		const colorSensorDropPosition: Position = {
		...
		const stage1Loop = async () => {
			...
			while (factoryRunning) {
				try {
					...
					console.log(
						`[Stage 1 #${stage1Count}] Moving to spawn position to grab cube...`,
					);
					await uarmGoToAndWait(
						uarm1,
						uarm1TD as WoT.ThingDescription,
						spawnPickPosition,
					);
					...
					console.log(
						`[Stage 1 #${stage1Count}] Closing gripper to grab cube...`,
					);
					...
					console.log(
						`[Stage 1 #${stage1Count}] Moving above ConveyorBelt1...`,
					);
					await uarmGoToAndWait(
						uarm1,
						uarm1TD as WoT.ThingDescription,
						conveyor1AbovePosition,
					);

					console.log(
						`[Stage 1 #${stage1Count}] Moving down to place on ConveyorBelt1...`,
					);
					await uarmGoToAndWait(
						uarm1,
						uarm1TD as WoT.ThingDescription,
						conveyor1PlacePosition,
					);

					await new Promise((resolve) => setTimeout(resolve, 500)); // brief pause to ensure stability

					console.log(
						`[Stage 1 #${stage1Count}] Opening gripper to release cube...`,
					);
					await uarm1.invokeAction("gripOpen");

					console.log(
						`[Stage 1 #${stage1Count}] Moving back above ConveyorBelt1...`,
					);
					await uarmGoToAndWait(
						uarm1,
						uarm1TD as WoT.ThingDescription,
					...
					console.log(
						`[Stage 1 #${stage1Count}] Cube placed on ConveyorBelt1. Signaling Stage 2...`,
					);
					...
					console.log(
						`[Stage 1 #${stage1Count}] Returning to spawn position...`,
					);
					await uarmGoToAndWait(
						uarm1,
						uarm1TD as WoT.ThingDescription,
		...
		const stage2Loop = async () => {
			...
			const readDistance = async (
				...
				try {
					...
					const sensorUnit =
						sensor1TD?.properties?.objectDistance?.unit || "meter";
					...
					if (
						normalizeUnit(sensorUnit) === "millimeter" &&
						rawDistance > 0 &&
						rawDistance < 10
					) {
						if (DEBUG)
							console.log(
								`[DEBUG] Heuristic override: Treated ${rawDistance} as meters despite unit=${sensorUnit}`,
							);
						return rawDistance;
					}
			...
			while (factoryRunning) {
				try {
					...
					if (sensor2) {
						console.log(
							`[Stage 2 #${stage2Count}] Waiting for sensor2 to detect cube...`,
						);
						...
						while (!detected && factoryRunning) {
							...
							if (presence) {
								detected = true;
								console.log(
									`[Stage 2 #${stage2Count}] sensor2: objectPresence=true! Cube detected.`,
								);
							}
							if (!detected) {
								await sleep(100);
					 ...
					 */
					console.log(
						`[Stage 2 #${stage2Count}] Uarm2 moving to home position...`,
					);
					const uarm2ReadyPromise = (async () => {
						await uarm2.invokeAction("gripOpen");
						await moveTo(
							uarm2,
							uarm2TD as WoT.ThingDescription,
							uarm2HomePosition,
						);
						console.log(
							`[Stage 2 #${stage2Count}] Uarm2 ready at home position.`,
						);
					})();

					console.log(
						`[Stage 2 #${stage2Count}] Monitoring sensor1 for objectPresence=true AND objectDistance≈${TARGET_DISTANCE}m...`,
					);

					let pickupReady = false;
					let checkCount = 0;
					const maxChecks = 200;
					let actualStopDistance: number | null = null;
					let lastObservedDistance: number | null = null;
					...
					if (sensor1) {
						while (!pickupReady && factoryRunning && checkCount < maxChecks) {
							...
							if (presence && distance !== null) {
								...
								console.log(
									`[Stage 2 #${stage2Count}] sensor1: presence=true, distance=${distance.toFixed(4)}m (target: ${TARGET_DISTANCE}m ±${DISTANCE_TOLERANCE}m)`,
								);
								...
								if (
									...
									console.log(
										`[Stage 2 #${stage2Count}] ✓ Perfect position! objectPresence=true, objectDistance=${distance.toFixed(4)}m`,
									);
								...
								console.log(
									`[Stage 2 #${stage2Count}] sensor1: presence=true, distance=unavailable`,
								);
							} else if (!presence) {
								if (checkCount % 20 === 0) {
									console.log(
										`[Stage 2 #${stage2Count}] sensor1: waiting for objectPresence...`,
									);
								}
							}
						...
						if (!pickupReady && factoryRunning) {
							console.log(
								`[Stage 2 #${stage2Count}] Timeout waiting for optimal position (${checkCount}/${maxChecks} checks). Proceeding anyway.`,
							);
						}
					} else {
						console.log(
							`[Stage 2 #${stage2Count}] No sensor1 available. Using time delay.`,
						);
						await sleep(2000);
					}
					...
					if (actualStopDistance === null && lastObservedDistance !== null) {
						...
						console.log(
							`[Stage 2 #${stage2Count}] Using last observed sensor1 distance for correction: ${actualStopDistance.toFixed(4)}m`,
						);
					}
					...
					console.log(
						`[Stage 2 #${stage2Count}] Waiting for cube to stabilize...`,
					);
					await sleep(1500);
					...
					if (sensor1) {
						...
						if (finalDistance !== null) {
							...
							console.log(
								`[Stage 2 #${stage2Count}] Final stop distance measured: ${actualStopDistance.toFixed(4)}m`,
							);
						}
					}
					 ...
					 */
					console.log(
						`[Stage 2 #${stage2Count}] ⚙️ Uarm2 picking cube from ConveyorBelt1...`,
					);
					...
					if (actualStopDistance !== null) {
						console.log(
							`[DEBUG_STAGE2] Sensor1 Unit in TD: ${sensor1TD?.properties?.objectDistance?.unit}, actualStopDistance: ${actualStopDistance}`,
						);
						const distanceDeviation =
							actualStopDistance - EXPECTED_PICKUP_DISTANCE;
						...
						console.log(
							`[Stage 2 #${stage2Count}] Adjusted pickup X: ${conveyor1PickPosition.x.toFixed(3)} → ${adjustedPickPosition.x.toFixed(3)} (distance deviation: ${distanceDeviation.toFixed(4)}m, offset: ${xOffset.toFixed(4)}m)`,
						);
					} else {
						console.log(
							`[Stage 2 #${stage2Count}] No valid sensor1 distance captured; using default pickup position.`,
						);
					}

					// Move down to grab position using adjusted position
					console.log(
						`[Stage 2 #${stage2Count}]   → Moving to grab position...`,
					);
					await moveTo(
						uarm2,
						uarm2TD as WoT.ThingDescription,
					...
					console.log(
						`[Stage 2 #${stage2Count}]   → Lifting cube to home position...`,
					);
					await moveTo(uarm2, uarm2TD as WoT.ThingDescription, {
						...adjustedPickPosition,
						z: 1.17,
					});
					...
					console.log(
						`[Stage 2 #${stage2Count}] Transferring cube to ConveyorBelt2...`,
					);
					await uarmGoToAndWait(
						uarm2,
						uarm2TD as WoT.ThingDescription,
					...
					while (!stage3ReadyForCube && factoryRunning) {
						if (Date.now() % 2000 < 50) {
							console.log(
								`[Stage 2 #${stage2Count}] Waiting for Stage 3 to be ready...`,
							);
						}
						await sleep(100);
					}
					...
					console.log(
						`[Stage 2 #${stage2Count}]   → Starting ConveyorBelt2...`,
					);
					await conveyor2.invokeAction("startBeltForward");

					// Move back slightly after releasing
					console.log(
						`[Stage 2 #${stage2Count}]   → Moving back after release...`,
					);
					await moveTo(uarm2, uarm2TD as WoT.ThingDescription, {
						...conveyor2DropPosition,
					...
					console.log(
						`[Stage 2 #${stage2Count}] Transfer complete. Signaling Stage 1 for next cube.`,
					);
		...
		const stage3Loop = async () => {
			...
			const readDistance = async (
				...
				try {
					...
					const sensorUnit =
						sensor3TD?.properties?.objectDistance?.unit || "meter";
					...
					if (
						...
						if (DEBUG)
							console.log(
								`[DEBUG] Stage3 Heuristic: Treated ${rawDistance} as meters`,
							);
						return rawDistance;
					}
			...
			while (factoryRunning) {
				try {
					...
					if (sensor4) {
						console.log(
							`[Stage 3 #${stage3Count}] Waiting for sensor4 (or sensor3) to detect cube...`,
						);
						...
						while (!detected && factoryRunning) {
							...
							if (presence4 || presence3) {
								...
								console.log(
									`[Stage 3 #${stage3Count}] Cube detected! (s4=${presence4}, s3=${presence3})`,
								);
							}
							if (!detected) {
								await sleep(100);
					...
					console.log(
						`[Stage 3 #${stage3Count}] UR3 moving to ready position above CB2...`,
					);
					const ur3ReadyPromise = (async () => {
						await ur3.invokeAction("openGripper");
						await moveTo(ur3, ur3TD as WoT.ThingDescription, {
							...conveyor2PickPosition,
							z: conveyor2PickPosition.z + 0.1,
						});
						console.log(
							`[Stage 3 #${stage3Count}] UR3 ready above pickup position.`,
						);
					})();

					console.log(
						`[Stage 3 #${stage3Count}] Monitoring sensor3 for objectPresence=true AND objectDistance≈${TARGET_DISTANCE_CB2}m...`,
					);

					let pickupReady = false;
					let sensor3DistanceAtStop: number | null = null;
					let checkCount = 0;
					const maxChecks = 200;
					...
					if (sensor3) {
						while (!pickupReady && factoryRunning && checkCount < maxChecks) {
							...
							if (presence && distance !== null) {
								...
								console.log(
									`[Stage 3 #${stage3Count}] sensor3: presence=true, distance=${distance.toFixed(4)}m (target: ${TARGET_DISTANCE_CB2}m ±${DISTANCE_TOLERANCE_CB2}m)`,
								);
								...
								if (
									...
									console.log(
										`[Stage 3 #${stage3Count}] ✓ Perfect position! objectPresence=true, objectDistance=${distance.toFixed(4)}m`,
									);
									pickupReady = true;
								...
								console.log(
									`[Stage 3 #${stage3Count}] sensor3: presence=true, distance=unavailable`,
								);
							} else if (!presence) {
								if (checkCount % 20 === 0) {
									console.log(
										`[Stage 3 #${stage3Count}] sensor3: waiting for objectPresence...`,
									);
								}
							}
						...
						if (!pickupReady && factoryRunning) {
							console.log(
								`[Stage 3 #${stage3Count}] Timeout waiting for optimal position (${checkCount}/${maxChecks} checks). Proceeding anyway.`,
							);
						}
					} else {
						console.log(
							`[Stage 3 #${stage3Count}] No sensor3 available. Using time delay.`,
						);
						await sleep(2000);
						stage3ReadyForCube = false; // Mark as busy
					}
					...
					// Wait for belt to fully stop and cube to stabilize
					console.log(
						`[Stage 3 #${stage3Count}] Waiting for cube to stabilize...`,
					);
					await sleep(1500);

					// PHASE 4: Pick cube with UR3 (uses 6-coordinate system)
					console.log(
						`[Stage 3 #${stage3Count}] ⚙️ UR3 picking cube from ConveyorBelt2...`,
					);
					...
					if (sensor3DistanceAtStop !== null) {
						...
						console.log(
							`[Stage 3 #${stage3Count}] Adjusted pickup Y: ${conveyor2PickPosition.y.toFixed(3)} → ${adjustedConveyor2PickPosition.y.toFixed(3)} (distance deviation: ${deviation.toFixed(4)}m, offset: ${yOffset.toFixed(4)}m)`,
						);
					} else {
						console.log(
							`[Stage 3 #${stage3Count}] No valid sensor3 distance captured; using default UR3 pickup position.`,
						);
					}

					// Move down to grab position
					console.log(
						`[Stage 3 #${stage3Count}]   → Moving to grab position...`,
					);
					await moveTo(
						ur3,
						ur3TD as WoT.ThingDescription,
						adjustedConveyor2PickPosition,
					);
					...
					console.log(
						`[Stage 3 #${stage3Count}]   → Moving to intermediate position...`,
					);
					await moveTo(
						ur3,
						ur3TD as WoT.ThingDescription,
						ur3IntermediatePosition,
					);

					// Move to color sensor position to hold cube for detection
					console.log(
						`[Stage 3 #${stage3Count}]   → Moving to color sensor position...`,
					);
					await moveTo(
						ur3,
						ur3TD as WoT.ThingDescription,
					...
					console.log(
						`[Stage 3 #${stage3Count}] Holding cube in front of sensor, waiting for detection...`,
					);

					console.log(
						`[Stage 3 #${stage3Count}] Waiting for color sensor to detect cube...`,
					);
					...
					while (!sensorDetected && detectionAttempts < maxDetectionAttempts) {
						const presenceValue =
							await colorSensor.readProperty("objectPresence");
					...
					if (!sensorDetected) {
						console.log(
							`[Stage 3 #${stage3Count}] Warning: Color sensor did not detect presence. Proceeding anyway.`,
						);
					}
					...
					console.log(
						`[Stage 3 #${stage3Count}] Detected cube color: ${color}`,
					);

					// Determine final position based on detected color
					let finalDropPosition: Position;
					let skipCube = false;
					...
					if (color === "red" && redCount < maxCubesPerColor) {
						...
						console.log(
							`[Stage 3 #${stage3Count}] Skipping cube: color=${color} (quota reached or unknown)`,
						);
						skipCube = true;
					...
					if (color === "green" && !skipCube) {
						console.log(
							`[Stage 3 #${stage3Count}]   → Moving to intermediate position for green cube...`,
						);
						await moveTo(
							ur3,
							ur3TD as WoT.ThingDescription,
						...
						// Open gripper to release cube
						console.log(
							`[Stage 3 #${stage3Count}]   → Releasing green cube at intermediate position...`,
						);
						await ur3.invokeAction("openGripper");
						await sleep(300);
					} else {
						...
						console.log(
							`[Stage 3 #${stage3Count}]   → Moving to ${color} drop position (y=${finalDropPosition.y.toFixed(3)})...`,
						);
						await moveTo(ur3, ur3TD as WoT.ThingDescription, finalDropPosition);

						// Open gripper to release cube
						console.log(
							`[Stage 3 #${stage3Count}]   → Releasing cube at ${color} position...`,
						);
						await ur3.invokeAction("openGripper");
						await sleep(300);
						...
						// Move up
						console.log(
							`[Stage 3 #${stage3Count}]   → Moving up after release...`,
						);
						await moveTo(ur3, ur3TD as WoT.ThingDescription, {
							...finalDropPosition,
						...
						console.log(
							`[Stage 3 #${stage3Count}]   → Returning to intermediate position...`,
						);
						await moveTo(
							ur3,
							ur3TD as WoT.ThingDescription,
					...
					if (!skipCube) {
						console.log(
							`[Stage 3 #${stage3Count}] Cube sorted to ${color} position. Counts: red=${redCount}, blue=${blueCount}, green=${greenCount}`,
						);
					}

					// Check if all quotas met
					if (
						redCount >= maxCubesPerColor &&
						blueCount >= maxCubesPerColor &&
						greenCount >= maxCubesPerColor
					) {
						console.log(
							`[Stage 3 #${stage3Count}] All color quotas met. Stopping factory...`,
						);
